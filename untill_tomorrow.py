...
# datetime.date を datetime.datetime にした
tomorrow = datetime.datetime.combine(tomorrow, datetime.time())
untill_tomorrow = (tomorrow -
...
class Timer(Tk.Frame):
    ...
    def start(self):  # Startを押したときの動作
        self.started = True
        self.finish = time.time() + int(untill_tomorrow)
        self.count()
    ...
    def stop(self):  # 停止処理
        self.started = False
        self.tokei.config(text=untill_tomorrow)
